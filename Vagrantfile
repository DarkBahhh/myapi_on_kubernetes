...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "nasnfs" do |nasnfs|
    ...
    nasnfs.vm.network :forwarded_port, guest: 22, host: 5551
    nasnfs.vm.network "private_network", ip: "10.0.3.2"
  end
  config.vm.define "master" do |master|
    master.vm.hostname = "master"
    master.vm.provision :shell, path: "./master.sh"
    master.vm.network :forwarded_port, guest: 22, host: 5552
  ...
  config.vm.define "node1" do |node1|
    ...
    node1.vm.network :forwarded_port, guest: 22, host: 5553
    node1.vm.network "private_network", ip: "10.0.3.4"
  end
  config.vm.define "node2" do |node2|
    node2.vm.hostname = "node2"
    node2.vm.provision :shell, path: "./worker.sh"
    node2.vm.network :forwarded_port, guest: 22, host: 5554
